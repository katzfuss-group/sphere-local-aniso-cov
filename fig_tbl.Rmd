...
## Plot prediction errors of nonsta-true 

```{r plot errors of nonsta-true, eval = F}
typeTrue <- "nonsta"
typeMdls <- c("iso", "axially", "nonsta") 
RDNames <- grep(paste0("rect-", type, "*.RData"), dir("./results/"), 
                value = T)
unknownTypes <- c("rect", "rand")
for(unknownType in unknownTypes)
{
  for(type in typeMdls){
    fnTrue <- paste0("results/", unknownType, "-", typeTrue, "-true.RData")
    fn <- paste0("results/", unknownType, "-", typeTrue, "-", type, 
                 "-pred.RData")
    load(fnTrue)
    inputTrue <- input
    load(fn)
    input$z.all <- input$z.all - inputTrue$z.all
    input$draw.palette <- T
    if(length(grep("results/", input$fn)) == 0)
      input$fn <- paste0("results/", input$fn)
    do.call(sphere_plot, input)
